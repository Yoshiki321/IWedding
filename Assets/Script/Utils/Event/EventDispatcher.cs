...
public class EventDispatcher
{
    ...
    public bool dispatchEvent(EventObject evt)
    {
        ...
        for (i = branchLength - 1; i > 0; i--)
        {
            ...
            if (_object.captureListeners != null && _object.captureListeners.Count > 0)
            {
                if (_object.captureListeners.ContainsKey(evt.type))
                {
                    vector = _object.captureListeners[evt.type];
                    if (vector != null)
                    {
                        length = vector.Count;
        ...
        for (i = 0; i < branchLength; i++)
        {
            ...
            if (_object.bubbleListeners != null && _object.bubbleListeners.Count > 0)
            {
                if (_object.bubbleListeners.ContainsKey(evt.type))
                {
                    vector = _object.bubbleListeners[evt.type];
                    if (vector != null)
                    {
                        length = vector.Count;
                        functions = new List<EventDelegate>();
