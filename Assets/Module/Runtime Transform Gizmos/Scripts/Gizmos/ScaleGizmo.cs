...
namespace RTEditor
{
    ...
    public class ScaleGizmo : Gizmo
    {
        ...
        protected override void OnInputDeviceFirstButtonDown()
        {
            ...
            InputDevice.Instance.GetPosition(out inputDevPos);
            _lastPoint = inputDevPos;
        }

        public bool isControlleManipulation;
        ...
        protected override bool DetectHoveredComponents(bool updateCompStates)
        {
            ...
            if (updateCompStates)
            {
                ...
                float tt;
                if (IsScaleAlongAllAxesShActive && pickRay.IntersectsBox(1.3f, 1.3f, 1.3f, GetScaleCentreBoxesWorldTransforms(), out tt))
                {
                    _isAllAxesSquareSelected = true;
        ...
        private Matrix4x4 GetScaleCentreBoxesWorldTransforms()
        {
            Matrix4x4 worldTransforms = new Matrix4x4();
            Vector3 worldPosition = _gizmoTransform.position;
            Quaternion worldRotation = _gizmoTransform.rotation;

            // Construct the world transform matrix
            worldTransforms.SetTRS(worldPosition, worldRotation, Vector3.Scale(_gizmoTransform.lossyScale, new Vector3(_scaleBoxWidth, _scaleBoxHeight, _scaleBoxDepth)));
        ...
        private void DrawAllAxesSquareLines()
        {
            ...
            if (IsScaleAlongAllAxesShActive)
            {
                ...
                //Vector2 screenSpaceSquareCenter = _camera.WorldToScreenPoint(_gizmoTransform.position);
                //GLPrimitives.Draw2DFilledRectangle(new Rect(screenSpaceSquareCenter.x - _screenSizeOfAllAxesSquare / 2, screenSpaceSquareCenter.y - _screenSizeOfAllAxesSquare / 2,
                //    _screenSizeOfAllAxesSquare, _screenSizeOfAllAxesSquare), squareLineColor, MaterialPool.Instance.GizmoLine, _camera);
                ...
                Mesh squareMesh = ProceduralMeshGenerator.CreateBoxMesh(1.3f, 1.3f, 1.3f);

                material.SetColor("_Color", Color.white);
                material.SetPass(0);
                Graphics.DrawMeshNow(squareMesh, GetScaleCentreBoxesWorldTransforms());
                material.SetInt("_CullMode", cullMode);
            }
        }
                ...
                if (topParents.Count != 0)
                {
                    ...
                    foreach (GameObject topParent in topParents)
                    {
                        if (topParent != null)
                        {
                            ...
                            Item3D item3 = topParent.GetComponent<Item3D>();

                            if (item3 == null || !item3.ScaleEnabled)
                            {
                                continue;
                            }
