        ...
        private void UpdateMousePoint()
        {
            ...
            if (topParents.Count != 0)
            {
                foreach (GameObject topParent in topParents)
                {
                    ...
                    if (item3 != null)
                    {
                        item3.mousePoint = topParent.transform.position;
                    }
                }
            }
        }
                ...
                if (topParents.Count != 0)
                {
                    ...
                    foreach (GameObject topParent in topParents)
                    {
                        ...
                        if (topParent != null)
                        {
                            ...
                            if (item3 == null || !item3.TranslationEnabled)
                            {
                                continue;
                            }

                            item3.mousePoint += moveVector;

                            //List<Item3D> list = new List<Item3D>();
                            //RelationVO rvo = (item3.VO as ItemVO).GetComponentVO<RelationVO>();
                            //if (rvo != null)
                            //{
                            //    foreach (ObjectData data in rvo.relationItems)
                            //    {
                            //        list.Add(data.object3 as Item3D);
                            //        (data.object3 as Item3D).mousePoint += moveVector;
                            //    }
                            //}

                            if (item3.mousePoint.y <= 0f && item3.mousePoint.y >= -1f)
                            {
                                topParent.transform.position = new Vector3(item3.mousePoint.x, 0, item3.mousePoint.z);

                                //foreach (Item3D item in list)
                                //{
                                //    item.transform.position = new Vector3(item.mousePoint.x, item.transform.position.y, item.mousePoint.z);
                                //}
                            }
                            else
                            {
                                topParent.transform.position = item3.mousePoint;

                                //foreach (Item3D item in list)
                                //{
                                //    item.transform.position = item.mousePoint;
                                //}
                            }
