...
namespace RTEditor
{
    ...
    public class EditorGizmoSystem : MonoSingletonBase<EditorGizmoSystem>, IMessageListener
    {
        ...
        public bool IsActiveGizmoReadyForObjectManipulation()
        {
            ...
            //为了translation和scale合并
            return (_translationGizmo.IsReadyForObjectManipulation()
                || _scaleGizmo.IsReadyForObjectManipulation()
                || _rotationGizmo.IsReadyForObjectManipulation());
        }
        ...
        private void Update()
        {
            ...
            _translationGizmo.isEnabled = !_scaleGizmo.isControlleManipulation;
            if (_translationGizmo.isEnabled) _translationGizmo.isEnabled = !_rotationGizmo.isControlleManipulation;
            _scaleGizmo.isEnabled = !_rotationGizmo.isControlleManipulation;

            if (_translationGizmo.gameObject.activeSelf)
            {
                if (_transformPivotPoint == TransformPivotPoint.MeshPivot && EditorObjectSelection.Instance.LastSelectedGameObject != null)
                    _scaleGizmo.transform.position = EditorObjectSelection.Instance.LastSelectedGameObject.transform.position;
                else
                    _scaleGizmo.transform.position = EditorObjectSelection.Instance.GetSelectionWorldCenter();

                if (_transformPivotPoint == TransformPivotPoint.MeshPivot && EditorObjectSelection.Instance.LastSelectedGameObject != null)
                    _rotationGizmo.transform.position = EditorObjectSelection.Instance.LastSelectedGameObject.transform.position;
                else
                    _rotationGizmo.transform.position = EditorObjectSelection.Instance.GetSelectionWorldCenter();

                if (EditorObjectSelection.Instance.LastSelectedGameObject != null)
                    _scaleGizmo.transform.rotation = EditorObjectSelection.Instance.LastSelectedGameObject.transform.rotation;
            }
        }
        ...
        /// </summary>
        public void EstablishActiveGizmoPosition()
        {
            EditorObjectSelection objectSelection = EditorObjectSelection.Instance;
            if (_activeGizmo.GetGizmoType() != GizmoType.VolumeScale && _activeGizmo != null /*&& !_activeGizmo.IsTransformingObjects()*/) // TODO: Why was this here? Seems that it is not necessary...
            {
                ...
                //为了translation和scale合并
                if (_transformPivotPoint == TransformPivotPoint.MeshPivot && objectSelection.LastSelectedGameObject != null) _translationGizmo.transform.position = objectSelection.LastSelectedGameObject.transform.position;
                else _translationGizmo.transform.position = objectSelection.GetSelectionWorldCenter();
                if (_transformPivotPoint == TransformPivotPoint.MeshPivot && objectSelection.LastSelectedGameObject != null) _scaleGizmo.transform.position = objectSelection.LastSelectedGameObject.transform.position;
                else _scaleGizmo.transform.position = objectSelection.GetSelectionWorldCenter();
                if (_transformPivotPoint == TransformPivotPoint.MeshPivot && objectSelection.LastSelectedGameObject != null) _rotationGizmo.transform.position = objectSelection.LastSelectedGameObject.transform.position;
                else _rotationGizmo.transform.position = objectSelection.GetSelectionWorldCenter();
            }
        ...
        private void UpdateActiveGizmoRotation()
        {
            ...
            else
            {
                ...
                //为了translation和scale合并
                if ((_transformSpace == TransformSpace.Global && _activeGizmoType != GizmoType.Scale) || objectSelection.LastSelectedGameObject == null)
                    _rotationGizmo.transform.rotation = Quaternion.identity;
                else
                    _rotationGizmo.transform.rotation = objectSelection.LastSelectedGameObject.transform.rotation;
                if ((_transformSpace == TransformSpace.Global && _activeGizmoType != GizmoType.Scale) || objectSelection.LastSelectedGameObject == null)
                    _translationGizmo.transform.rotation = Quaternion.identity;
                else
                    _translationGizmo.transform.rotation = objectSelection.LastSelectedGameObject.transform.rotation;

                if (objectSelection.LastSelectedGameObject != null)
                    _scaleGizmo.transform.rotation = objectSelection.LastSelectedGameObject.transform.rotation;
            }
        }
        #endregion
        ...
        {
            return item.GetComponent<Item3D>() == null;
        }
        ...
        private void OnSelectionChanged(ObjectSelectionChangedEventArgs selChangedEventArgs)
        {
            ...
            if (!_areGizmosTurnedOff)
            {
                ...
                if (objectSelection.NumberOfSelectedObjects != 0 && !_activeGizmo.gameObject.activeSelf)
                {
                    ...
                    _scaleGizmo.gameObject.SetActive(true);
                    _rotationGizmo.gameObject.SetActive(true);
                }
            }
