...
namespace RTEditor
{
    public class GameObjectSphereTree
    {
        ...
        public void Update()
        {
            ...
            Item3D[] sceneObjects = MonoBehaviour.FindObjectsOfType<Item3D>();
            foreach (Item3D item3 in sceneObjects)
            {
                if (!IsGameObjectRegistered(item3.gameObject)) 
                {
                    if (!RegisterGameObject(item3.gameObject)) continue;
                }

                #if !USE_TRANSFORM_HAS_CHANGED
                Transform objectTransform = item3.gameObject.transform;
                GameObjectTransformData objectTransformData = _gameObjectToTransformData[item3.gameObject];
                   ...
                   objectTransformData.Scale != objectTransform.lossyScale)
                {
                    ...
                    _gameObjectToTransformData[item3.gameObject] = GetGameObjectTransformData(item3.gameObject);
                }
                #else
